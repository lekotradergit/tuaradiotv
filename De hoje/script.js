...
const playerToggleBtn = document.getElementById('mainPlayPauseBtn');
...
window.tocarRadio = function(url, nome, pais, countryCode, cardElement) {
    ...
    if (abaSecundariaAtiva === 'recentes') {
        atualizarListaSecundaria();
        const oFocoEstaNasRecentes = secondaryRadioList && secondaryRadioList.contains(document.activeElement);
        
        if (oFocoEstaNasRecentes) {
            setTimeout(() => {
                const primeiroCardSec = secondaryRadioList.querySelector('.radio-row-card');
                if (primeiroCardSec) {
                    primeiroCardSec.focus();
                    primeiroCardSec.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
                }
            }, 50);
        }
    }
...
document.addEventListener('keydown', (e) => {
    ...
    // Gestão da barra lateral (.sidebar-tv) com base estritamente na ordem vertical dos elementos
    if (focused.closest('.sidebar-tv')) {
        const sidebarElements = Array.from(document.querySelectorAll('.sidebar-tv select, .sidebar-tv input, .sidebar-tv button')).filter(el => el.offsetParent !== null);
