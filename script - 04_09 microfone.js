...
function criarCartaoHTML(radio, isFav, estaTocando) {
    ...
    // CORREÇÃO: O clique do rato passa a acionar a lógica de alternância (Play/Pause)
...
// Prepara o código do país para formatar a bandeira de forma limpa
    ...
    audioPlayer.play().then(() => {
        if (playerStatus) playerStatus.textContent = "No AR";
        
        // Atualiza o nome da rádio no título principal do player
        if (playingTitle) playingTitle.textContent = radioNome;
        
        // Atualiza os metadados (incluindo a bandeira e o país) mantendo a estrutura original
        if (songMetadata) {
...
if (!SpeechRecognition) {
    ...
    if (micBtn) micBtn.style.display = 'none'; // Oculta o botão caso o navegador não seja compatível
} else {
    const recognition = new SpeechRecognition();
    recognition.lang = 'pt-BR'; // Altere para 'pt-PT' se preferir o português de Portugal
    recognition.interimResults = false;
    recognition.maxAlternatives = 1;

    // Guarda o texto HTML original do botão para poder restaurá-lo depois
    const originalButtonContent = micBtn.innerHTML;

    // 1. Evento de clique para iniciar a escuta
    ...
    // 2. Quando o microfone começa a gravar
    recognition.onstart = () => {
        micBtn.innerHTML = "🎤 Fale agora...";
        micBtn.classList.add('voice-listening');
    };

    // 3. Quando a voz é capturada e convertida em texto
    recognition.onresult = (event) => {
        const speechToText = event.results[0][0].transcript;
        
        if (searchInput) {
            // Insere o texto falado na caixa de pesquisa
            searchInput.value = speechToText;
            
            // Atualiza o estado visual do filtro (borda ativa, etc.)
            verificarFiltrosAtivos();
            
            // Dispara diretamente a função de pesquisa da aplicação
            pesquisarRadios(false);
        }
    };

    // 4. Tratamento de erros de captura
    recognition.onerror = (event) => {
        console.error("Erro no reconhecimento de voz: ", event.error);
        restaurarBotao();
    };

    // 5. Quando o processo de escuta termina
    recognition.onend = () => {
        restaurarBotao();
    };

    // Função auxiliar para devolver o botão ao estado original
...
// Gestão de Readonly e Teclado Virtual no Campo de Pesquisa
if (searchInput) {
