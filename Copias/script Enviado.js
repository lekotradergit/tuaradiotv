...
window.tocarRadio = function(url, nome, pais, countryCode, cardElement) {
    ...
    if (abaSecundariaAtiva === 'recentes') {
        ...
        // Verifica se o cartão clicado veio da lista de pesquisa
        const veioDaPesquisa = cardElement && searchRadioList && searchRadioList.contains(cardElement);

        // SÓ forçamos o foco para o topo dos recentes se o utilizador NÃO veio da pesquisa
        if (!veioDaPesquisa) {
            setTimeout(() => {
...
// ==========================================
if (playerToggleBtn) {
    // Função unificada para alternar entre Play e Pausa
    const alternarReproducaoPlayer = () => {
        // Se não houver nenhuma rádio carregada mas existirem recentes, carrega a primeira automaticamente
        ...
        if (!audioPlayer.src) {
            return; // Se não houver nenhuma rádio selecionada, não faz nada
        }

        if (audioPlayer.paused) {
            audioPlayer.play().then(() => {
                playerToggleBtn.textContent = "⏸"; // Altera para o símbolo de pausa
                if (playerStatus) playerStatus.textContent = "No Air";
                atualizarEstadosVisuaisNasListas();
            }).catch(err => {
            ...
            audioPlayer.pause();
            playerToggleBtn.textContent = "▶"; // Altera para o símbolo de play
            if (playerStatus) playerStatus.textContent = "Pausado";
            atualizarEstadosVisuaisNasListas();
        }
    };
    ...
    // Evento de teclado (essencial para o comando da TV ao pressionar Enter)
